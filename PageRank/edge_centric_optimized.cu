...
#include <fstream>
#include <math.h>
#include "kernels/EdgeCentric_phase1_optmized.h"
...
void PageRank_edge_centric(Graph* G,int iter,float df,int blocksPerGrid,int threadsPerBlock){
    ...
    while(iter--){
        ...
        EdgeCentric_phase1_optmized<<<grid, block>>>(d_newpr,d_oldpr,d_sinkArray, G->N,G->countSink,df);
...
int main(){
    ...
    char output_file[] = "edgeCentricOptimized_output.txt";
